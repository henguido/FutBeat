...
// Three official requests per explicit import. No automatic retries or live polling.
export async function fetchCostaRica({ fetcher = fetch } = {}) {
  const result = {};
  for (const [key, endpoint] of [
    ['league', 'lookupleague.php?id=4815'],
    ['next', 'eventsnextleague.php?id=4815'],
    ['past', 'eventspastleague.php?id=4815'],
  ]) {
    const response = await fetcher(base + endpoint, { signal: AbortSignal.timeout(15000) });
...
export async function normalize(raw, resolve, receivedAt) {
  ...
  for (const envelope of [raw.past, raw.next]) {
    ...
    for (const event of envelope.events ?? []) {
      ...
      for (const side of ['Home', 'Away']) {
        ...
        teamIds.push(teamId);
        teams.set(teamId, { id: teamId, name: required(event[`str${side}Team`], 'team name'), country: 'Costa Rica', competitionId, media: null, aliases: [] });
      }
  ...
  return { schemaVersion: 1, demo: false, updatedAt: receivedAt, coverage, competitions: [competition], teams: [...teams.values()], matches: [...matches.values()], players: [], standings: [], news: [], transfers: [] };
}
