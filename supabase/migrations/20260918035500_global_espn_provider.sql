-- Add ESPN as a second central global fixture provider.
-- Keeps the same canonical identity and shared snapshot merge contract.
...
declare
  ...
  matches integer;
begin
  if p_provider not in ('sofascore','espn')
...
begin
  ...
  if exists(
    select 1
     ...
     where m->'provenance'->>'source' not in ('SofaScore','ESPN')
  ) then
  ...
  merged := p_snapshot || jsonb_build_object(
    ...
    jsonb_build_object(
      ...
      'sources',jsonb_build_array('TheSportsDB','API-Football','SofaScore','ESPN')
    ),
    'competitions',(
    ...
    'matches',(
      ...
      from (
        select value
         ...
         where not (
           old.value->'provenance'->>'source' in ('SofaScore','ESPN')
