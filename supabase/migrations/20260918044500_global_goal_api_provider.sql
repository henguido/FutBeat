-- Add GOAL API as a licensed central global fixture provider.
-- No competition allow-list: every fixture returned for the date window is eligible.
...
begin
  ...
  merged := p_snapshot || jsonb_build_object(
    ...
    jsonb_build_object(
      ...
      'sources',jsonb_build_array('TheSportsDB','API-Football','SofaScore','ESPN','GOAL API')
    ),
    'competitions',(
    ...
    'matches',(
      ...
      from (
        select value
         ...
         where not (
           old.value->'provenance'->>'source' in ('SofaScore','ESPN','GOAL API')
