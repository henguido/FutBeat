-- Expire active status stored in the canonical entity when no fresh layer can
-- confirm it. Reconcile every calendar row from the canonical entity, the durable LIVE
...
begin
  ...
  select coalesce(
    jsonb_agg(
      case
        when resolved.status is not null then entity.payload ||
          jsonb_build_object(
  ...
  left join lateral (
    ...
    from (
      values
        (
          case
            when entity.payload->>'status' in ('LIVE','HALFTIME','EXTRA_TIME','PENALTIES')
             and coalesce(
               nullif(entity.payload->'provenance'->>'receivedAt','')::timestamptz,
               calendar.updated_at
             )<now()-interval '15 minutes'
            then 'SCHEDULED'
            else entity.payload->>'status'
          end,
          case
            when entity.payload->>'status' in ('LIVE','HALFTIME','EXTRA_TIME','PENALTIES')
             and coalesce(
            ...
            then null
            when coalesce(entity.payload->>'minute','') ~ '^\d+$'
            then (entity.payload->>'minute')::integer else null
          end,
          case
            when entity.payload->>'status' in ('LIVE','HALFTIME','EXTRA_TIME','PENALTIES')
             and coalesce(
