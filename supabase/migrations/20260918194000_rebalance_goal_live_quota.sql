-- Protect GOAL API daily quota using the provider-reported remaining count.
-- API-Football polling is retired now that GOAL LIVE is verified end-to-end.
...
begin
  ...
  if v_last_remaining is not null and v_last_remaining<=120 then
...
begin
  ...
  into v_job_id
  from cron.job
  where jobname='futbeat-live-sync-free-tier';
