...
El bloque 0.2 añade el proveedor TheSportsDB y persistencia local. El bloque 0.3 conecta la lectura remota en Supabase y un APK mediante HTTPS. El bloque LIVE añade eventos canónicos por Realtime, recuperación HTTP y una outbox push privada e idempotente. Country Bootstrap + Favorites-First usa Costa Rica como cobertura inicial y agrega prioridades sin llamadas por usuario. Consulta [datos locales](docs/DATA-BLOCK.md), [conexión en la nube](docs/CLOUD.md), [LIVE/push](docs/LIVE-PUSH.md) y [país/favoritos](docs/COUNTRY-FAVORITES.md). La demo continúa siendo el modo predeterminado al compilar sin configuración.
